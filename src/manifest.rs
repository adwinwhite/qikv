...
impl ManifestKeeper {
    pub fn new(store_dir: &Path) -> Result<ManifestKeeper> {
        let init_current = MANIFEST_SNAPSHOT_PREFIX.to_owned() + "_0" 
            + "\n" 
    ...
    pub fn snapshot(&mut self, store_dir: &Path) -> Result<()> {
        ...
        let mut snapshot_file = File::options().write(true).create(true).truncate(true).open(store_dir.join(MANIFEST_SNAPSHOT_PREFIX.to_owned() + "_" + &snapshot_num.to_string()))?;
        let log_file = File::options().write(true).create(true).truncate(true).open(store_dir.join(MANIFEST_LOG_PREFIX.to_owned() + "_" + &log_num.to_string()))?;
                ...
                if let Some(path_str) = path.to_str() {
                    if let Ok(level) = path_str.parse::<u64>() {
                        match manifest.active_ssts.get(&level) {
                            Some(ids) => {
                                for sst in fs::read_dir(store_dir.join(SSTABLE_DIR).join(level.to_string()))? {
                                    let sst = sst?;
    ...
    pub fn next_compact(&mut self, level: u64) {
        self.batch.push(ManifestAction::NextCompact((level, )));
    }

    pub fn add(&mut self, sst_id: SstId, first_key: &[u8], last_key: &[u8]) {
        self.batch.push(ManifestAction::Add((sst_id, first_key.to_vec(), last_key.to_vec(),)));
    }

    pub fn remove(&mut self, sst_id: &SstId) {
        self.batch.push(ManifestAction::Remove((sst_id.clone(),)));
    }

    pub fn new_id(&mut self, level: u64) {
        self.batch.push(ManifestAction::NewId((level,)));
    }
    ...
    pub fn commit(&mut self) -> Result<()> {
        ...
        bincode::encode_into_std_write(ManifestAction::Commit, &mut self.log, bincode::config::standard())?;
        self.log.sync_all()?;

        // Apply changes to in-memory manifest.
        let internal = &mut self.manifest;
        for action in &self.batch {
            internal.execute_action(action);
            if let ManifestAction::Remove((sst_id,)) = action {
                SSTable::remove(&self.store_dir, sst_id)?;
            }
        }
        self.batch.clear();
        Ok(())
...
impl Manifest {
    ...
    pub fn latest_compact_sst(&self, level: u64) -> SstId {
        ...
        let latest_start = match self.compact_keys.get(&level) {
            ...
            None => {
                &self.sst_ranges.first_key_value().unwrap().1.0
            }
        };
    ...
    pub fn new_sst_id(&mut self, level: u64) -> SstId {
        let id = self.new_ids.entry(level).and_modify(|i| { *i += 1 }).or_insert(1);
        SstId {
            level,
            id: *id,
        }
    ...
    pub fn execute_action(&mut self, action: &ManifestAction) {
        match action {
            ManifestAction::Commit => {},
            ManifestAction::NextCompact((level,)) => { self.next_compact_sst(*level); },
            ManifestAction::Add((sst_id, first_key, last_key,)) => { self.add_sst(*sst_id, &first_key, &last_key); },
            ManifestAction::Remove((sst_id,)) => { self.remove_sst(&sst_id); },
            ManifestAction::NewId((level,)) => { self.new_sst_id(*level); },
        }
    }

    pub fn sort(&self, sst_ids: &[SstId]) -> Vec<SstId> {
        let mut metas: Vec<_> = sst_ids.iter()
            .map(|sst_id| 
                SSTMetadata { 
                    level: sst_id.level,
                    id: sst_id.id,
            ...
            .collect();
        metas.sort();
        metas.iter().map(|m| SstId { level: m.level, id: m.id }).collect()
    }
}
...
mod tests {
    ...
    fn test_normal_exit() -> Result<()> {
        ...
        actions.push(ManifestAction::Add((SstId { level: 0, id: 0}, key_range.0, key_range.1,)));

        actions.push(ManifestAction::NewId((0,)));
        let key_range = get_random_key_range(16, 17);
        actions.push(ManifestAction::Add((SstId { level: 0, id: 1}, key_range.0, key_range.1,)));

        actions.push(ManifestAction::NewId((0,)));
        let key_range = get_random_key_range(16, 17);
        actions.push(ManifestAction::Add((SstId { level: 0, id: 2}, key_range.0, key_range.1,)));

        actions.push(ManifestAction::NewId((0,)));
        let key_range = get_random_key_range(16, 17);
        actions.push(ManifestAction::Add((SstId { level: 0, id: 3}, key_range.0, key_range.1,)));
        // Compact to level 1
        actions.push(ManifestAction::NextCompact((1,)));
        actions.push(ManifestAction::NewId((1,)));
        let key_range = get_random_key_range(16, 17);
        actions.push(ManifestAction::Add((SstId { level: 1, id: 0}, key_range.0, key_range.1,)));
        for i in 0..4 {
        ...
        ensure!(keeper0.eq(&keeper), "(Normal Exit) Recovered manifest is corrupted");
    ...
    fn test_new_id() -> Result<()> {
        ...
        for j in 0..4 {
            for i in 0..4 {
                keeper.batch_start();
                ensure!(keeper.latest_sst_id(j) == SstId { level: j, id: i}, "Assigned sst id is wrong. it should be level={j}, id={i}");
                keeper.new_id(j);
                keeper.commit()?;
            }
        }
