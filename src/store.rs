...
impl Store {
    ...
    pub fn get(&self, key: &[u8]) -> Option<Vec<u8>> {
        ...
        match self.memtable.get(&key.to_vec()) {
            ...
            None    => {
                let group = SSTGroup::new(&self.manifest.get_sst_by_key(&key), &self.dir).expect("Failed to load SSTable"); 
                match group.get(&key) {
        ...
        if level_ids.len() == 0 {
            ...
            if level == 0 {
                if level_ids.len() >= 4 {
                    ...
                    SSTGroup::new(&overlappings, &self.dir)?.compact(1, &self.dir, &mut self.manifest)?;
                ...
                if self.manifest.level_byte_size(level, &self.dir)? > u64::pow(10, level as u32) * u64::pow(2, 20) {
                    ...
                    SSTGroup::new(&overlappings, &self.dir)?.compact(level + 1, &self.dir, &mut self.manifest)?;
...
mod tests {
    ...
    fn test_complete() -> Result<()> {
        ...
        for (k, v) in &good_map {
            if &store.get(k).ok_or(anyhow!("Store is missing some pair"))? != v {
